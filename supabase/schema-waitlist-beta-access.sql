-- =============================================================================
-- AgentCloud — Waitlist Beta Access (Phase 1: Migration)
-- =============================================================================
-- Run this file in Supabase SQL Editor (idempotent).
--
-- Tables:
--   1. waitlist_codes       — redemption codes for beta access
--   2. waitlist_redemptions — ledger of who redeemed what
--   Alter:
--   3. profiles             — add `role` column for beta_tester / internal_qa
--
-- Env flag: ENABLE_WAITLIST_BETA_BYPASS=true|false
--   When false, all bypass checks no-op (treat as normal member).
-- =============================================================================


-- -----------------------------------------------------------------------------
-- 0. Guard: only run if env flag is set (checked via current_setting)
--    If the GUC is not set, the migration still runs (tables are idempotent).
-- -----------------------------------------------------------------------------
