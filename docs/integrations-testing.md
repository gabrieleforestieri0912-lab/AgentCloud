...
| **Google Sheets** | Test spreadsheet under existing Google test user (`78915...`) | `accounts.google.com/o/oauth2/v2/auth?scope=spreadsheets` + `oauth2.googleapis.com/token` + refresh coded, `GOOGLE_SCOPES` now includes `gmail.modify calendar spreadsheets` and synced to Vercel, `google-sheets-proxy` `sheets.googleapis.com/v4` coded | Re-consent Google user with added `spreadsheets` scope (`/api/integrations/google_sheets/authorize`; whitelistare in Google Console il redirect `/api/integrations/google_sheets/callback`), verify `tenant_integrations` row, then use the agent tools `sheets_read_range` / `sheets_update_range` / `sheets_append_row` on a test spreadsheet |
