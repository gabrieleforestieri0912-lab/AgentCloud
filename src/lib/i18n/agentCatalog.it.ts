/**
 * Overlay italiano del catalogo agenti (slug → campi di display localizzati).
 *
 * Il contenuto canonico vive in `src/lib/agents.ts` in inglese; questi dati lo
 * sovrascrivono tramite `localizeAgent()` quando la lingua attiva è l'italiano.
 * Il badge NON è qui: è una proprietà canonica dell'agente e viene tradotta da
 * `localizeBadge()` in `./agentCatalog.ts`.
 */
